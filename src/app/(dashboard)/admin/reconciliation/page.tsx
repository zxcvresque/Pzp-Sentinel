...
export default function ReconciliationPage() {
  ...
  return <div>
    ...
    <Section title="Unmatched BMC payments" empty="No unmatched BMC payments.">
      {data.unmatchedBmc.map((transaction) => <Row key={transaction.id} title={`${transaction.currency} ${Number(transaction.amount).toLocaleString()} · ${transaction.bmcWebhookEvents[0]?.supporterName || "Unknown supporter"}`} detail={`${transaction.description} · ${new Date(transaction.date).toLocaleString()}`}><Link href="/admin/transactions" className="pill">Assign donor</Link></Row>)}
    </Section>
