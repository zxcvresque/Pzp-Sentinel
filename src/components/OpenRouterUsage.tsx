...
export default function OpenRouterUsage({ role }: { role: "ADMIN" | "DEV" }) {
  ...
  return <div className="mx-auto max-w-[1500px]"><ServicesNav role={role} />
    ...
    {role === "ADMIN" && (accounts.length ? <div className="space-y-8">{accounts.map((account) => { const credits = account.snapshot?.credits; const activity = account.snapshot?.activity || []; const workspace = account.snapshot?.workspaces?.find((item) => item.id === account.workspaceId); const totalRequests = activity.reduce((sum, row) => sum + Number(row.requests || 0), 0); const totalTokens = activity.reduce((sum, row) => sum + Number(row.prompt_tokens || 0) + Number(row.completion_tokens || 0) + Number(row.reasoning_tokens || 0), 0); return <section key={account.id} data-share-target={`openrouter-account:${account.id}`} className="space-y-5"><div className="overflow-hidden rounded-2xl border border-lime/25 bg-gradient-to-br from-lime/[.11] via-bg-card to-violet/[.07] p-6 shadow-[0_24px_90px_rgba(111,209,215,.06)]"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono text-[9px] uppercase tracking-[.16em] text-lime">Account-level usage · always visible</p><h2 className="mt-1 text-2xl font-extrabold">{account.name}</h2><p className="mt-1 text-xs text-text-tertiary">Management sync {stamp(account.lastFetchedAt)} · {workspace ? `${workspace.name} workspace` : account.workspaceId ? `workspace ${account.workspaceId}` : "OpenRouter default workspace"}</p></div><ShareButton entityType="openrouter-account" entityId={account.id} contextTitle={account.name} contextDetails={`${money(credits?.total_usage)} used · ${money(credits?.remaining)} remaining`} /></div><div className="mt-6 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><div><p className="font-mono text-[9px] uppercase tracking-[.14em] text-text-tertiary">Total OpenRouter usage</p><div className="mt-1 flex items-end justify-between gap-3"><strong className="font-display text-5xl text-lime">{money(credits?.total_usage)}</strong><span className="text-right text-xs text-text-tertiary">{money(credits?.remaining)} remaining<br />of {money(credits?.total_credits)} purchased</span></div><div className="mt-4"><Progress used={Number(credits?.total_usage || 0)} limit={Number(credits?.total_credits || 0) || null} /></div></div><div className="grid grid-cols-2 gap-2"><Metric label="30d requests" value={compact(totalRequests)} accent /><Metric label="30d tokens" value={compact(totalTokens)} /><Metric label="Provider keys" value={account.snapshot?.keys?.length || 0} /><Metric label="Tracked in Sentinel" value={account.keys.length} /></div></div>{account.lastError && <p className="mt-4 rounded-lg bg-coral/[.06] p-3 text-xs text-coral">Latest sync: {account.lastError}. Showing the last good snapshot.</p>}</div>
    ...
    </section>; })}</div> : !showAccountForm ? <div className="card p-10 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl border border-lime/20 bg-lime/[.06] text-lime"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg></div><h2 className="mt-4 text-lg font-bold">Connect your first OpenRouter account</h2><p className="mx-auto mt-2 max-w-lg text-sm text-text-tertiary">Sentinel will discover your workspace, then show purchased credits, usage, limits and per-key accounting without exposing the management credential.</p><button onClick={() => setShowAccountForm(true)} className="mt-5 rounded-full bg-lime px-5 py-2.5 text-sm font-bold text-bg-void">Connect account</button></div> : null)}
  </div>;
}
