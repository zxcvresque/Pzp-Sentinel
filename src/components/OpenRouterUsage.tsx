...
export default function OpenRouterUsage({ role }: { role: "ADMIN" | "DEV" }) {
  ...
  return <div className="mx-auto max-w-[1500px]"><ServicesNav role={role} />
    ...
    {showAccountForm && <form onSubmit={addAccount} className="card mb-6 border-lime/15 p-5"><div className="mb-4"><h2 className="font-bold">Connect an OpenRouter account</h2><p className="mt-1 text-xs text-text-tertiary">Use a Management API key. It is validated server-side, AES-256-GCM encrypted, and never shown again.</p></div><div className="grid gap-3 md:grid-cols-3"><label className="text-xs text-text-tertiary">Account name<input required value={accountForm.name} onChange={(event) => setAccountForm({ ...accountForm, name: event.target.value })} className="input mt-1.5" /></label><label className="text-xs text-text-tertiary">Management API key<input required type="password" autoComplete="off" value={accountForm.managementKey} onChange={(event) => setAccountForm({ ...accountForm, managementKey: event.target.value })} className="input mt-1.5" /></label><label className="text-xs text-text-tertiary">Workspace ID (optional)<input value={accountForm.workspaceId} onChange={(event) => setAccountForm({ ...accountForm, workspaceId: event.target.value })} className="input mt-1.5" /></label></div><div className="mt-4 flex justify-end"><button disabled={saving} className="rounded-full bg-lime px-5 py-2 text-sm font-bold text-bg-void">{saving ? "Validating…" : "Connect securely"}</button></div></form>}
