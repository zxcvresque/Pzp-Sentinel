...
const DONOR: Slide[] = [
  slide("01 · Your perks", "A token of our gratitude", "Access perks are for monthly supporters. A substantial one-time contribution may qualify for one month at the admins’ discretion. Perks are not automatic or guaranteed.", "#FBBF24", [], [["Stremio & Jellyfin", "Stremio and/or Jellyfin access as a thank-you for your support.", "server"], ["Custom roles", "A community role, subject to community rules.", "people"], ["Gratitude XP", "XP in Sentry after a reviewer approves your contribution.", "approval"], ["More community perks", "Other perks as they become available.", "gift"]]),
  slide("02 · Contribute", "Choose how you want to support", "Give once or monthly through Razorpay or BMC, or submit an outside contribution with proof.", "#6FD1D7", [["My Donations", "/donor"], ["Checkout", "/donor#razorpay"], ["Manual proof", "/donor#manual"]], [["Razorpay", "UPI, cards and supported wallets.", "payment"], ["Buy Me a Coffee", "A simple USD option for international donors.", "coffee"], ["Monthly support", "Autopay records every renewal automatically under your name.", "clock"], ["Manual contribution", "Attach proof for admin review.", "receipt"]], "PAYMENTS"),
  slide("03 · Track", "History, corrections and totals", "Understand every contribution and fix a pending or rejected submission without starting again.", "#FBBF24", [["History", "/donor#history"], ["Leaderboard", "/donor#leaderboard"]], [["Clear history", "Provider, status, proof and receipt together.", "ledger"], ["Pending changes", "Edit, cancel or add missing proof before review.", "action"], ["Appeals", "Reply with context when a submission is rejected.", "message"], ["Fair totals", "Original currency plus a comparable INR total.", "chart"]], "PAYMENTS"),
  slide("04 · Control", "Privacy and preferences", "Choose how Sentinel contacts you while financial proof remains private to you and authorized admins.", "#A78BFA", [["Settings", "/profile"]], [["Notifications", "Choose contribution events and delivery channels.", "bell"], ["Reminders", "Control the nudges you want to receive.", "clock"], ["Private proof", "Financial files use owner/admin authorization.", "secure"]]),
];
...
export default function RoleOnboarding({ role, name, photoUrl, githubUsername, requireGithub = role === "DEV", onComplete }: Props) {
  ...
  const perksPage = role === "DONOR" && index === 0;
  ...
  return (
    <div className="onboarding-shell fixed inset-0 z-[10000] overflow-hidden bg-[#090a0f]" role="dialog" aria-modal="true" aria-label={`Welcome to Sentinel for ${role.toLowerCase()}s`}>
      ...
      <div className="relative mx-auto flex h-dvh max-w-[1540px] flex-col overflow-hidden px-3 py-2.5 sm:px-6 sm:py-4 lg:px-8">
        ...
        <main className="grid min-h-0 flex-1 place-items-center py-2 sm:py-4">
          {githubStage ? <GitHubStage accent={current.accent} value={github} error={githubError} onChange={(value) => { setGithub(value); setGithubError(""); }} /> :
            <section key={`${role}-${index}`} className={`workspace-card onboarding-${direction} relative grid w-full max-h-full min-h-0 gap-3 rounded-[22px] border border-white/10 bg-[#14151d]/92 p-3 shadow-[0_30px_90px_rgba(0,0,0,.38)] backdrop-blur-xl sm:gap-5 sm:p-5 lg:p-8 ${reverse ? "lg:grid-cols-[minmax(540px,1.16fr)_minmax(0,.84fr)]" : "lg:grid-cols-[minmax(0,.84fr)_minmax(540px,1.16fr)]"}`}>
              ...
              <div className={`workspace-preview relative min-w-0 ${reverse ? "lg:order-1" : ""}`}>
                ...
                <div className={`relative grid grid-cols-1 gap-2 rounded-[18px] border border-white/8 bg-black/20 p-2 sm:gap-3 sm:p-3 ${current.items.length === 3 ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
                  {current.items.map((item, itemIndex) => <article key={item.title} className="workspace-item relative flex min-h-[58px] items-start gap-2.5 rounded-xl border border-white/8 bg-white/[.04] p-2.5 text-left sm:min-h-[98px] sm:gap-3 sm:p-3.5 lg:min-h-[124px] lg:p-5"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg sm:h-8 sm:w-8 lg:h-9 lg:w-9" style={{ color: current.accent, background: `${current.accent}16` }}><FeatureIcon label={item.icon} /></span><div className="min-w-0 pr-1"><h2 className="text-[9px] font-bold leading-[1.25] text-white/88 sm:text-[12px] lg:text-[13px]">{item.title}</h2><p className="workspace-item-detail mt-1 line-clamp-2 text-[8px] leading-[1.35] text-white/50 sm:text-[10px] sm:leading-[1.4] lg:line-clamp-none lg:text-[11px] lg:leading-[1.55]">{item.detail}</p></div><span className="absolute right-2 top-2 font-mono text-[7px] text-white/18">{String(itemIndex + 1).padStart(2, "0")}</span></article>)}
                </div>
              </div>
            </section>}
        </main>
...
function GitHubStage({ accent, value, error, onChange }: { accent: string; value: string; error: string; onChange: (value: string) => void }) {
  return <div className="grid h-full place-items-center"><section className="w-full max-w-2xl rounded-[24px] border border-white/10 bg-white/[.04] p-5 text-center shadow-2xl sm:p-8"><Image src="/github-mark.svg" alt="GitHub" width={72} height={72} className="mx-auto h-12 w-12 sm:h-16 sm:w-16" /><p className="mt-3 font-mono text-[8px] uppercase tracking-[.2em] sm:text-[9px]" style={{ color: accent }}>One-time developer setup</p><h1 className="mt-2 text-2xl font-black tracking-[-.03em] text-white sm:text-4xl">Connect your GitHub identity</h1><p className="mx-auto mt-2 max-w-lg text-[10px] leading-4 text-white/52 sm:text-sm sm:leading-5">Enter this carefully. Sentinel uses this exact GitHub account for repository access, issue assignments, pull requests and contribution activity—not to sign in.</p><p className="mx-auto mt-2 max-w-lg text-[9px] font-semibold leading-4 text-amber sm:text-xs">Make sure the username is correct before continuing; future project and repository assignments will use it.</p><label className="mx-auto mt-4 block max-w-md text-left"><span className="sr-only">GitHub username</span><div className="flex items-center rounded-2xl border border-white/14 bg-black/30 px-4 focus-within:border-white/40"><span className="text-xs text-white/35 sm:text-sm">github.com/</span><input autoFocus value={value} onChange={(event) => onChange(event.target.value.replace(/^@/, ""))} maxLength={39} autoComplete="username" placeholder="your-username" className="min-w-0 flex-1 bg-transparent px-1 py-3 text-xs font-bold text-white outline-none placeholder:text-white/20 sm:text-sm" /></div>{error && <span className="mt-1.5 hidden text-[10px] leading-4 text-red-300 sm:block">{error}</span>}</label></section></div>;
}
